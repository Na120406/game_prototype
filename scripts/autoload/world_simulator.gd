			...
			for schedule_raw: Variant in schedules:
				...
				if schedule.get("day_of_week", -1) == (day - 1) % 7:
					...
					if schedule.get("type", "") == "mountain":
						...
						var configured_vos_schedule: Dictionary = {}
						if npc_id == "shopkeeper_father" and NPCSchedules.has_method("get_vos_mountain_schedule_for_day"):
							configured_vos_schedule = NPCSchedules.call("get_vos_mountain_schedule_for_day", day)
						...
						if not configured_vos_schedule.is_empty():
							context["event_day"] = int(configured_vos_schedule.get("event_day", day))
							context["departure_time"] = float(configured_vos_schedule.get("departure_time", 11.0))
							context["fall_time"] = float(configured_vos_schedule.get("fall_time", 16.0))
							context["end_time"] = float(configured_vos_schedule.get("end_time", 17.0))
						...
						var is_configured_vos_event: bool = not configured_vos_schedule.is_empty()
						if risk >= 0.3 or (is_configured_vos_event and not player_escorted):
