...
## =============================================================================
## Interactive object cho phép player thu thập item một lần (persistent).
## Sau khi thu thập, object biến mất và không respawn — state lưu qua
...
func _ready() -> void:
	...
	if gathering_id != "" and GameState.is_gathering_collected(gathering_id):
...
func interact(_player_ref: Node) -> void:
	...
	# Đánh dấu đã thu thập (persistent)
	if gathering_id != "":
		GameState.mark_gathering_collected(gathering_id)
